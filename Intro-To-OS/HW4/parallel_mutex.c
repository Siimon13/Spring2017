/*
1) Insert was inserting into multiple ones and causing conflicts

3)Add locks to the retreive so only one retrieve can happen without overlap

5)Add locks to insert so only one insert can happen without overlap

Runtime: 2 cores
6.308082 seconds (Takes longer because it has to lock cores)
...
pthread_mutex_t lock2;	//declare a lock2 for retriving
...
void insert(int key, int val) {
  ...
  pthread_mutex_lock(&lock); //Acquire the lock for lock
  e->next = table[i];
  e->key = key;
  e->val = val;
  table[i] = e;
...
bucket_entry * retrieve(int key) {
    ...
    for (b = table[key % NUM_BUCKETS]; b != NULL; b = b->next) {
      ...
      if (b->key == key) {
	pthread_mutex_lock(&lock2); //Acquire the lock 
	pthread_mutex_unlock(&lock2); //Unlocks the mutex
	return b;
...
int main(int argc, char **argv) {
    ...
    pthread_mutex_init(&lock, (pthread_mutexattr_t *)NULL); //Initiliazes the lock for atrributes	    
    ...
    pthread_mutex_init(&lock2, (pthread_mutexattr_t *)NULL); //Initiliazes the lock for atrributes	        
    for (i = 0; i < num_threads; i++) {
