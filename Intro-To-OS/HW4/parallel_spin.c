...
int keys[NUM_KEYS];
pthread_spinlock_t spinlock;	//declare a lock
...
void insert(int key, int val) {
  ...
  pthread_spin_lock(&spinlock); //Acquire the lock for lock
  e->next = table[i];
  e->key = key;
  e->val = val;
  table[i] = e;
  pthread_spin_unlock(&spinlock); //Unlocks the mutex
}
...
int main(int argc, char **argv) {
    ...
    pthread_spin_init(&spinlock, 0); //Initiliazes the lock for atrributes
